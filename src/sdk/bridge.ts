...
export function mapSdkMessage(message: SDKMessage): EventInput[] {
  switch (message.type) {
    ...
    case "result": {
      // Reported unconditionally -- once per `result`, success or error alike
      // -- since what a Turn cost is real whether or not it produced a diff or
      // finished cleanly. `repo` is filled in by the caller, which has the
      // project directory this pure mapper does not.
      const usageEvent: EventInput = {
        type: "usage",
